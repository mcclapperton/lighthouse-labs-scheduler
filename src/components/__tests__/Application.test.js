...
describe("Application", () => {
  ...
  it("loads data, edits an interview and keeps the spots remaining for Monday the same", async () => {
    ...
    // 4. Switch the student name
    fireEvent.change(getByPlaceholderText(appointment, /enter student name/i), {
      target: { value: "Maris Clapperton" },
    });
    // 5. Click the "Save" button.
    fireEvent.click(queryByText(appointment, "Save"));
    // 6. Check that the element with the text "Saving!" is displayed.
    expect(queryByText(appointment, "Saving!")).toBeInTheDocument();
    // 7. Wait until the element is back
    await waitForElement(() => getByText(container, "Maris Clapperton"));
    expect(getByText(appointment, "Maris Clapperton")).toBeInTheDocument();
  ...
  it("shows the save error when failing to save an appointment", async () => {
    ...
    fireEvent.click(queryByText(appointment, "Save"));
    // 5. check if displays save erorr message
    ...
    // 7. same amount of spots remaining
    const day = getAllByTestId(container, "day").find((day) =>
      queryByText(day, "Monday")
    );
    expect(getByText(day, "1 spot remaining")).toBeInTheDocument();
  ...
  it("shows the delete error when failing to delete an existing appointment", async () => {
    ...
    fireEvent.click(queryByText(appointment, "Confirm"));
    // 5. check if displays save error message
    await waitForElement(() =>
